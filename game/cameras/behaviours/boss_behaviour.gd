...
## With 0.0 the camera will be exactly over the boss.
## With 1.0 the camera will be exactly over the player.
...
func update_camera(camera: GameCamera) -> void:
	if not camera.player or not _boss:
		camera.switch_to_default_behaviour()
		return

	var direction: Vector3 = camera.player.global_position - _boss.global_position
	direction.y = 0.0
	...
	var expanded_player_position: Vector3 = camera.player.global_position + player_offset
	var expanded_boss_position: Vector3 = _boss.global_position + boss_offset

	var center: Vector3 = (expanded_player_position + expanded_boss_position) * player_boss_position_ratio
