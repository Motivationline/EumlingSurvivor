...
func _create_textures(color_format: RenderingDevice.DataFormat, size: Vector2i) -> void:
	...
	color_texture = rd.texture_create(half_screen_texture, RDTextureView.new())
	...
	for i in range(1, screen_textures.size()):
		screen_textures[i] = rd.texture_create_shared_from_slice(RDTextureView.new(), color_texture, 0, i - 1) # create texture views into mip levels
...
func _create_pipeline(shader: RID, framebuffer_format: int, blend_attachment: RDPipelineColorBlendStateAttachment = RDPipelineColorBlendStateAttachment.new()) -> RID:
...
func _initialize_compute() -> void:
	...
	var shader_file: RDShaderFile = load("res://post_process/depth_of_field/depth_of_field_mip.glsl")
...
func _render_callback(p_effect_callback_type: EffectCallbackType, p_render_data: RenderData) -> void:
	...
	for view in view_count:
		...
		if not downsample_pipeline.is_valid():
			downsample_pipeline = _create_pipeline(downsample_shader, input_framebuffer_format)
		...
		for i in range(1 , screen_textures.size()):
			...
			rd.draw_list_bind_render_pipeline(downsample_draw_list, downsample_pipeline)
			rd.draw_list_set_push_constant(downsample_draw_list, push_constant.to_byte_array(), push_constant.size() * 4)
			rd.draw_list_bind_uniform_set(downsample_draw_list, UniformSetCacheRD.get_cache(downsample_shader, 0, [color_uniform]), 0)
		...
		color_texture_uniform.add_id(color_texture)
