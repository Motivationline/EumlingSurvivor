...
func find_new_target():
	...
	match target:
		target_types.PLAYER:
			tp_position = get_tree().get_nodes_in_group("Player")[0].global_position() + (Vector3(randf_range(-1, 1),0.0,randf_range(-1, 1)).normalized() * random_offset)
		
		target_types.RANDOM:
			var random_position := Vector3.ZERO
			while get_point_on_map(random_position, 0) == Vector3.ZERO:
				...
				tp_position = get_point_on_map(random_position, 0)
	
		target_types.TEAMMEMBER:
			var group: Array[StringName] = self.get_groups()
			...
			if len(targets) > 1:
				...
				while true:
					...
					if not member == parent:
						break
				tp_position = member.global_position()
		
		target_types.BACK:
			var back_position = parent.position + (parent.global_transform.basis.z * back)
			tp_position = get_point_on_map(back_position,0.2)
